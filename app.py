...
# --- MOTOR DE DIAGNÓSTICO DE LIBRERÍAS ---
ERRORES = []
try:
    import pandas as pd
except ImportError:
    ERRORES.append("Falta 'pandas'")
try:
    import plotly.express as px
except ImportError:
    ERRORES.append("Falta 'plotly'")
try:
    import openpyxl
except ImportError:
    ERRORES.append("Falta 'openpyxl' (para leer Excel)")
...
def mostrar_landing():
    ...
    st.markdown("""<div class="footer">SIGD DINIC v28.0 | DESARROLLADO POR: <b>JSCN</b> | cnjstalin@gmail.com</div>""", unsafe_allow_html=True)

def mostrar_secretario():
    col_back, col_tools = st.columns([1.5, 6])
    with col_back: 
        if st.button("⬅ VOLVER"): navegar_a("landing")
    with col_tools:
        st.markdown(f'<div style="background:#263238; padding:10px; border-radius:8px; color:white; font-weight:bold; height:50px; display:flex; align-items:center; justify-content:center;">🛡️ GESTOR DOCUMENTAL DINIC</div>', unsafe_allow_html=True)
    ...
    with st.container():
        ...
        with c4: st.text_input("CARGO:")
        with c5: st.text_input("UNIDAD ORIGEN:")
...
# ------------------------------------------------------------------------------
# MÓDULO TALENTO HUMANO (DASHBOARD FIX)
# ------------------------------------------------------------------------------
def mostrar_th():
    ...
    if ERRORES:
        st.error(f"❌ El dashboard no puede cargar porque: {', '.join(ERRORES)}")
        st.warning("⚠️ Asegúrate de que tu requirements.txt tenga: streamlit, pandas, plotly, openpyxl")
        return
    ...
    if archivo:
        try:
            ...
            # DASHBOARD VISUAL
            st.markdown('<div class="sec-header">VISUALIZACIÓN DE DATOS</div>', unsafe_allow_html=True)
            k1, k2 = st.columns(2)
            k1.metric("Total de Personal", len(df))
            
            
            
            g1, g2 = st.columns(2)
            with g1:
                eje_x = st.selectbox("Analizar por (Barras):", df.columns)
                fig_bar = px.histogram(df, x=eje_x, title=f"Distribución por {eje_x}", color_discrete_sequence=['#0E2F44'], template="plotly_white")
                st.plotly_chart(fig_bar, use_container_width=True)
            
            with g2:
                eje_p = st.selectbox("Analizar por (Pastel):", df.columns, index=min(1, len(df.columns)-1))
                fig_pie = px.pie(df, names=eje_p, title=f"Porcentaje de {eje_p}", hole=0.4)
                st.plotly_chart(fig_pie, use_container_width=True)

            st.markdown('<div class="sec-header">TABLA DE DATOS</div>', unsafe_allow_html=True)
            st.dataframe(df, use_container_width=True)
            
        except Exception as e:
            st.error(f"Ocurrió un error al leer el archivo: {e}")
    else:
        st.info("ℹ️ Para activar el dashboard, primero debe subir un archivo Excel o CSV con los datos del personal.")
